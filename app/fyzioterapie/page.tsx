...
export default function FyzioterapiePage() {
  ...
  return (
    <main className="min-h-screen max-w-sm mx-auto pb-10">
      <BackHeader
        ...
        href="/"
        action={
          <Link href="/fyzioterapie/nova-schuze">
        ...
        <>
          <div className="mx-4 mb-5 bg-gray-50 rounded-2xl px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">
                {recording ? `Nahrávka · ${Math.floor((recording.duration_seconds || 0) / 60)} min` : 'bez nahrávky'}
              </p>
            </div>
            <button onClick={setNextSession} className="flex items-center gap-1 flex-shrink-0 active:opacity-60">
          ...
          {tasks.length > 0 && (
            <div className="px-4 mb-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {tasks.map(task => (
                  <Link key={task.id} href={`/fyzioterapie/${session.id}/ukol/${task.id}`}>
                    <div className="bg-white border rounded-2xl p-3.5 h-full active:scale-95 transition-transform"
                      style={{ borderColor: cfg.border }}>
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center mb-2.5 text-xs font-medium"
          ...
          {tasks.length === 0 && (
            <div className="mx-4 mb-5 p-4 rounded-2xl border border-dashed border-gray-200 text-center">
              ...
              <Link href={`/fyzioterapie/${session.id}/pridat-ukol`} className="text-xs mt-1 block" style={{ color: cfg.accent }}>
                + přidat úkol
              </Link>
            </div>
          )}
          ...
          <div className="px-4">
            <Link href="/fyzioterapie/historie">
  ...
  return (
    <div className="px-4 pt-10 text-center">
      ...
      <p className="text-sm text-gray-400 mb-6 max-w-xs mx-auto">
        Po první fyzioterapeutické schůzce tady přidej záznam s úkoly.
      </p>
      <Link href="/fyzioterapie/nova-schuze">
        <button className="px-6 py-3 rounded-2xl text-sm font-medium text-white active:opacity-80" style={{ background: cfg.accent }}>
